...
export default function DocumentsPage() {
  ...
  const filteredDocs = (Array.isArray(documents) ? documents : fallbackDocuments).filter(doc => {
    if (!doc) return false;
    const title = (doc.title || '').toLowerCase();
    const titleHi = (doc.title_hi || '').toLowerCase();
    const q = searchQuery.toLowerCase();
    const matchesSearch = title.includes(q) || titleHi.includes(q);
    const matchesCat = selectedCategory === 'All' || selectedCategory === 'सभी' || doc.category === selectedCategory;
    return matchesSearch && matchesCat;
  });

  const getFileBadgeClass = (fileType) => {
    const ft = (fileType || 'pdf').toLowerCase();
    if (ft.includes('pdf')) return { bg: '#FEF2F2', color: '#DC2626', label: 'PDF' };
    if (ft.includes('doc')) return { bg: '#EFF6FF', color: '#2563EB', label: 'DOC' };
    if (ft.includes('xls')) return { bg: '#F0FDF4', color: '#16A34A', label: 'XLS' };
    if (ft.includes('zip')) return { bg: '#FFF7ED', color: '#EA580C', label: 'ZIP' };
    return { bg: '#F5F3FF', color: '#7C3AED', label: ft.toUpperCase() };
  };
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
          {filteredDocs.map((doc, idx) => {
            const badge = getFileBadgeClass(doc.file_type);
