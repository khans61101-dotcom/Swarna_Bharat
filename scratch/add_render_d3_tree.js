...
function renderD3TreeDiagram(data) {
    ...
    try {
        ...
        // Parent SVG Container with D3 Zoom & Pan
...
const searchStr = 'renderD3TreeDiagram(rootData);\n}';
if (content.includes(searchStr)) {
    content = content.replace(searchStr, searchStr + d3TreeFunctionCode);
    fs.writeFileSync(target, content, 'utf8');
    console.log('SUCCESS: Inserted renderD3TreeDiagram into app.js!');
} else {
    console.log('Searching for regex match...');
    const regex = /(renderD3TreeDiagram\(rootData\);\s*\})/
    if (regex.test(content)) {
        content = content.replace(regex, '$1' + d3TreeFunctionCode);
        fs.writeFileSync(target, content, 'utf8');
        console.log('SUCCESS: Regex inserted renderD3TreeDiagram into app.js!');
    } else {
        console.error('FAILED to find insertion target in app.js');
    }
}
